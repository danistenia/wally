...
def find_wally_by_row_number(file_path: str, row_number: int) -> None:
    ...
    with open(file=file_path, mode="r") as file:
        ...
        if row_number > len(lines):
            raise ValueError(f"El número que indicaste en row_number: {row_number} es mayor que el número de lineas en el archivo.")
        else:
            line = lines[row_number-1]
            line_split = line.split()

            ImagePatches = namedtuple("ImagePatches", ["path", "wally_index", "x", "y", "w", "h"])
            img = ImagePatches(line_split[0], int(line_split[1]), int(line_split[2]), int(line_split[3]), int(line_split[4]), int(line_split[5]))
