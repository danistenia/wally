...
class PatchBuilder:
    ...
    def build_dict(self):
        ...
        self.patch_dict = {
            ...
            for k in range(0, round(y_stopper))
        }
    ...
    def build_patches(self):
        ...
        for idx, (y, y_prima) in enumerate(
            zip(
                range(0, self.img.y_length + self.square_area, self.square_area),
                range(
                    self.square_area,
                    self.img.y_length + self.square_area,
                    self.square_area,
                ),
            )
        ):

            for jdx, (x, x_prima) in enumerate(
                zip(
                    range(0, self.img.x_length + self.square_area, self.square_area),
                    range(
                        self.square_area,
                        self.img.x_length + self.square_area,
                        self.square_area,
                    ),
                )
            ):
